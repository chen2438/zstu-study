...
class Spider:
    def __init__(self, name):
        self.name = name
        self.url = (
            "https://openstd.samr.gov.cn/bzgk/gb/std_list?p.p1=0&p.p2="
            + name
    ...
    def get_each_link(self):
        ...
        while True:
            try:
                ...
                r = session.get(
                    self.url, headers=headers, verify=False
                )  # 请求数据,返回给Response对象
                ...
                Ids = selector.xpath(
                    '//tr/td[@style="text-align: left;"]/a/@onclick'
                )  # 选择结点
                print(Ids)
                ...
                for each_id in Ids:
                    ...
                    link = (
                        "https://openstd.samr.gov.cn/bzgk/gb/newGbInfo?hcno=" + each_id
                    )
                ...
                self.url = (
                    "https://openstd.samr.gov.cn/bzgk/gb/std_list?r=0.40987171406906286&page="
                    + str(index)
                    + "&pageSize=10&p.p1=0&p.p2="
                    + self.name
                    + "&p.p90=circulation_date&p.p91=desc"
                )
                print(self.url)
    ...
    def get_detial_from_linkList(self, link_list):
        ...
        for link in link_list:
            try:
                ...
                E_name = re.findall("<td>英文标准名称：(.+)</td>", html)[
                    0
                ]  # 这里使用正则表达式，因为xpath莫名其妙使用不了
                status = selector.xpath('//td[@align="left"]/span[@class]/text()')[
                    0
                ].strip()
                ccs = selector.xpath(
                    '//div[@class="col-xs-12 col-md-4 content"]/text()'
                )[
                    0
                ].strip()  # 中国标准分类号（CCS）
                ics = selector.xpath(
                ...
                release_date = selector.xpath(
                    '//div[@class="col-xs-12 col-md-4 content"]/text()'
                )[
                    2
                ].strip()  # 发布日期
                implement_date = selector.xpath(
                    '//div[@class="col-xs-12 col-md-4 content"]/text()'
                )[
                    3
                ].strip()  # 实施日期

...
class SimpleShell {
   private:
    ...
    void login(const std::vector<std::string>& args) {
        if (args.size() != 2) {
            std::cerr << "Usage: my-login <username> <password>" << std::endl;
            return;
        }
    ...
    void changePassword(const std::vector<std::string>& args) {
        if (args.size() != 2) {
            std::cerr << "Usage: my-passwd <username> <newpassword>" << std::endl;
            return;
        }
    ...
    void executeCommand(const std::string& command,
        ...
        if (command == "my-login") {
            ...
            if (fs.currentUser == nullptr) {
                ...
                return;
            } else if (command == "my-chmod") {
                if (args.size() != 2) {
                    std::cerr
                        << "Usage: my-chmod <permissions> <filename/directory>"
                        << std::endl;
                    return;
                }
                // 将数字权限转换为字符权限
            ...
            } else if (command == "my-getperm") {
                if (args.size() != 1) {
                    std::cerr << "Usage: my-getperm <filename/directory>"
                              << std::endl;
                    return;
                }
                std::cout << fs.getPermissions(args[0]) << std::endl;
            } else if (command == "my-pwd") {
                std::cout << fs.getCurrentDirectoryPath() << std::endl;
            } else if (command == "my-cd") {
                if (args.size() == 1) {
                    fs.changeDirectory(args[0]);
                } else {
                    std::cerr << "Usage: my-cd [directory]" << std::endl;
                    return;
                }
            } else if (command == "my-ls") {
                bool longFormat = !args.empty() && args[0] == "-l";
                fs.listDirectory(longFormat);
            } else if (command == "my-touch") {
                if (args.size() != 1) {
                    std::cerr << "Usage: my-touch <filename>" << std::endl;
                    return;
                }
                fs.create(args[0], false, fs.currentUser->username,
                          fs.currentUser->group);
            } else if (command == "my-rm") {
                if (args.size() != 1) {
                    std::cerr << "Usage: my-rm <filename/directory>" << std::endl;
                    return;
                }
                fs.remove(args[0]);
            } else if (command == "my-mkdir") {
                if (args.size() != 1) {
                    std::cerr << "Usage: my-mkdir <directory>" << std::endl;
                    return;
                }
                fs.create(args[0], true, fs.currentUser->username,
                          fs.currentUser->group);
            } else if (command == "my-rmdir") {
                if (args.size() != 1) {
                    std::cerr << "Usage: my-rmdir <directory>" << std::endl;
                    return;
                }
                fs.remove(args[0]);
            } else if (command == "my-cat") {
                if (args.size() != 1) {
                    std::cerr << "Usage: my-cat <filename>" << std::endl;
                    return;
                }
                fs.readFile(args[0]);
            } else if (command == "my-vim") {
                if (args.size() != 1) {
                    std::cerr << "Usage: my-vim <filename>" << std::endl;
                    return;
                }
                editFile(args[0]);
            } else if (command == "my-adduser") {
                if (args.size() != 2) {
                    std::cerr << "Usage: my-adduser <username> <password>"
                              << std::endl;
                    return;
                }
                fs.addUser(args[0], args[1], "users");  // 默认用户组为"users"
            } else if (command == "my-su") {
                if (args.size() != 2) {
                    std::cerr << "Usage: my-su <username> <password>" << std::endl;
                    return;
                }
                if (!fs.login(args[0], args[1])) {
            ...
            } else if (command == "my-passwd") {
                if (args.size() != 2) {
                    std::cerr << "Usage: my-passwd <username> <newpassword>"
                              << std::endl;
                    return;
                }
                fs.changeUserPassword(args[0], args[1]);

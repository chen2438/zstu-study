...
class User {
   public:
    ...
    User(const std::string& username, const std::string& group,
         const std::string& homeDirectory)
        : username(username), group(group), homeDirectory(homeDirectory) {}
};
...
class FileSystem {
   ...
   public:
    ...
    FileSystem() {
        ...
        // 添加root用户
        users.emplace(std::piecewise_construct, std::forward_as_tuple("root"),
                      std::forward_as_tuple("root", "root", "/root"));
    ...
    void executeCommand(const std::string& command,
        ...
        if (command == "login") {
            ...
            if (fs.currentUser == nullptr) {
                ...
                if (args.size() != 1) {
                    std::cerr << "Usage: adduser <username>" << std::endl;
                    return;
                }
                fs.addUser(args[0], "users");  // 默认用户组为"users"
            } else if (command == "su") {
                if (args.size() != 1) {
                    std::cerr << "Usage: su <username>" << std::endl;
                    return;
                }
                if (!fs.login(args[0])) {
